...
import { hastamalakaStotraVerses } from "@/data/hastamalaka-stotra"
...
export default function HastamalakaStotraPage() {
  ...
  const currentVerse = hastamalakaStotraVerses[selectedVerse]
  const progress = ((selectedVerse + 1) / hastamalakaStotraVerses.length) * 100

  // Load preferences from localStorage
  useEffect(() => {
    const savedFontSize = localStorage.getItem('hastamalaka-font-size')
    const savedViewMode = localStorage.getItem('hastamalaka-view-mode')
    const savedBookmarks = localStorage.getItem('hastamalaka-bookmarks')
  ...
  useEffect(() => {
    localStorage.setItem('hastamalaka-font-size', fontSize.toString())
    localStorage.setItem('hastamalaka-view-mode', viewMode)
    localStorage.setItem('hastamalaka-bookmarks', JSON.stringify(bookmarkedVerses))
  }, [fontSize, viewMode, bookmarkedVerses])

  const nextVerse = () => {
    if (selectedVerse < hastamalakaStotraVerses.length - 1) {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="max-w-4xl mx-auto text-center mb-12">
        <Badge className="mb-4 bg-[#e07c24] text-white">Self-Knowledge Expression</Badge>
        <h1 className="text-4xl md:text-5xl font-serif font-bold text-[#8b5d33] dark:text-[#e07c24] mb-4">
          Hastamalaka Stotra
        </h1>
        <p className="font-sanskrit text-2xl text-[#e07c24] mb-4">हस्तामलक स्तोत्रम्</p>
        <p className="text-lg text-[#5a4a3f] dark:text-[#d9c5a9] mb-6">
          The Song of <span className="font-semibold">Crystal-Clear Self-Knowledge</span>
        </p>
        <p className="text-md text-[#5a4a3f] dark:text-[#d9c5a9] leading-relaxed max-w-3xl mx-auto">
          A remarkable hymn traditionally attributed to Hastamalaka, a young disciple of Adi Shankaracharya who appeared intellectually disabled but possessed complete Self-knowledge. When Shankaracharya asked "Who are you?" the boy spontaneously sang these {hastamalakaStotraVerses.length} verses describing the nature of the Self with perfect clarity. The name "Hastamalaka" means "like an amalaka fruit in the palm"—referring to immediate, crystal-clear knowledge.
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto mb-12">
        <Card className="bg-white/80 dark:bg-[#2a241e]/80 border-[#e07c24]/20">
          ...
          <CardContent className="space-y-4 text-[#5a4a3f] dark:text-[#d9c5a9]">
            <p>
              <strong className="text-[#8b5d33] dark:text-[#e07c24]">Historical Context:</strong> Hastamalaka was a young boy who appeared intellectually disabled. When Shankaracharya asked "Who are you?" the boy spontaneously sang these verses revealing perfect Self-knowledge. Shankaracharya accepted him as a disciple and established one of the four mathas (monasteries) under his guidance. This demonstrates that Self-knowledge is not dependent on intellectual capacity or scholarship.
            </p>
            <p>
              <strong className="text-[#8b5d33] dark:text-[#e07c24]">Key Teaching:</strong> The Self is Pure Consciousness (Chit-rupa), auspicious (Shiva-rupa), non-dual Brahman—not the individual body-mind but the infinite, eternal, blissful awareness that is the reality of all. Each verse uses direct negation (neti-neti) of false identifications followed by positive assertion of truth.
            </p>
            <p>
              <strong className="text-[#8b5d33] dark:text-[#e07c24]">Unique Significance:</strong> Spontaneous expression of enlightenment from a young, unlearned person—demonstrating that Self-knowledge is not gradual acquisition but sudden recognition; not for the learned alone but available to all who are ripe for it; not in the future but in the eternal now.
            </p>
            <p>
              <strong className="text-[#8b5d33] dark:text-[#e07c24]">The Refrain:</strong> Each verse concludes with "Chid-rupa aham sada Shiva-rupa" (I am always of the nature of Pure Consciousness and Auspiciousness)—the constant affirmation of one's true nature as the limitless Self identical with Brahman.
            </p>
            <div className="flex flex-wrap gap-2 pt-4">
              <Badge variant="outline" className="border-[#e07c24]/30">Direct Knowledge</Badge>
              <Badge variant="outline" className="border-[#e07c24]/30">Pure Advaita</Badge>
              <Badge variant="outline" className="border-[#e07c24]/30">Self-Realization</Badge>
              <Badge variant="outline" className="border-[#e07c24]/30">Immediate Recognition</Badge>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="max-w-5xl mx-auto mb-8">
          ...
          <div className="mb-6">
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-[#8b5d33] dark:text-[#e07c24]">
                Progress: Verse {selectedVerse + 1} of {hastamalakaStotraVerses.length}
              </span>
        ...
        {viewMode === 'single' && (
          <div className="flex flex-wrap gap-2 mb-8 p-4 bg-[#f7f3e9]/50 dark:bg-[#1a1814]/50 rounded-lg">
            {hastamalakaStotraVerses.map((verse, idx) => (
              <button
                key={idx}
                onClick={() => {
                  setIsTransitioning(true)
        ...
        {viewMode === 'single' && (
          <>
            ...
            <div className="flex justify-between items-center mt-8">
                ...
                <div className="text-sm text-[#5a4a3f] dark:text-[#d9c5a9]">
                  {selectedVerse > 0 && (
                    <div className="mb-1 text-xs opacity-70">
                      ← Verse {hastamalakaStotraVerses[selectedVerse - 1].number}
                    </div>
                  )}
                  <div className="font-semibold text-[#8b5d33] dark:text-[#e07c24]">
                    {selectedVerse + 1} / {hastamalakaStotraVerses.length}
                  </div>
                  {selectedVerse < hastamalakaStotraVerses.length - 1 && (
              ...
              <Button
                onClick={nextVerse}
                disabled={selectedVerse === hastamalakaStotraVerses.length - 1}
                className="bg-[#e07c24] hover:bg-[#c06a1f] text-white disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Next
      ...
      <div className="max-w-4xl mx-auto mt-12">
        <Card className="bg-white/80 dark:bg-[#2a241e]/80 border-[#e07c24]/20">
          ...
          <CardContent className="space-y-3 text-[#5a4a3f] dark:text-[#d9c5a9]">
            <div>
              <strong>• Swami Paramarthananda's Lectures:</strong> Comprehensive verse-by-verse analysis with practical applications
            </div>
            <div>
              <strong>• Chinmaya Mission:</strong> Swami Chinmayananda's commentary on direct Self-knowledge
            </div>
            <div>
              <strong>• Advaita Ashrama:</strong> Ramakrishna Mission publications on Hastamalaka's spontaneous realization
            </div>
            <div>
