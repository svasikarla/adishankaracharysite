...
const latLngToXY = (lat: number, lng: number) => {
  // Calibrated coefficients based on linear regression of state centroids
  // x = 12.6254 * lng - 671.941
  // y = -21.1078 * lat + 921.164
  const x = 12.6254 * lng - 671.941
  const y = -21.1078 * lat + 921.164
  return { x, y }
}
    ...
    <div className="w-full">
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5">
          <div className="h-[900px] overflow-y-auto pr-2 space-y-4">
            {travelLocations.map((location, index) => {
              const isFirst = index === 0
              const isLast = index === travelLocations.length - 1
              const isMatha = ['sringeri', 'puri', 'dwarka', 'joshimath'].includes(location.id)
              ...
              return (
                <div
                  key={location.id}
                  className={`p-5 rounded-xl border-2 transition-all duration-300 ${hoveredLocation === location.id
